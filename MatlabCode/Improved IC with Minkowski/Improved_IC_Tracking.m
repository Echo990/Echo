...
for i=1:N-1
    ...
    e1(i+1) = x_sys(2)-y_r(i+1); 
    e2(i+1) = x_sys(3)-Theta_r(i+1);
    e0 = [e1(i+1);e2(i+1)];
end
%%
figure('Name','System state and Reference');
subplot 311
stairs(t(1:N-1),x_record(1,1:N-1),'b');
hold on
stairs(t,x_r,'r');
hold off
xlabel('t')
...
legend('x','x_r')
subplot 312
stairs(t(1:N-1),x_record(2,1:N-1),'b');
hold on
stairs(t,y_r,'r');
hold off
xlabel('t')
...
legend('y','y_r')
subplot 313
stairs(t(1:N-1),x_record(3,1:N-1),'b');
hold on
stairs(t,Theta_r,'r');
hold off
xlabel('t')
